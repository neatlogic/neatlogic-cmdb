...
public class DslSearchManager {
    ...
    private SelectFragment buildSelectFragment(SearchItem searchItem) {
        ...
        return new SelectFragment(select);
    }

    /**
     * 根据属性情况补充需要join的表
     */
    private void supplyJoinForSelectFragment(SearchItem searchItem, Select select) {
        SelectBody selectBody = select.getSelectBody();
        PlainSelect plainSelect = (PlainSelect) selectBody;
        ...
        if (MapUtils.isNotEmpty(this.searchItemMap)) {
            for (String key : this.searchItemMap.keySet()) {
                ...
                if (searchItem.getAttrVo() != null && !selectFragment.isAttrExists(searchItem.getAttrVo().getId())) {
                    ...
                    Select ciSelect = this.ciSelectMap.get(searchItem.getCiPath()).getSelect();
                    this.supplyJoinForSelectFragment(searchItem, ciSelect);
                }

                //如果前面有属性或关系，则需要额外生成cmdb_attrentity或cmdb_relentity的join语句
                String prevTable = "ci_base";
                String prevColumn = "id";
                if (CollectionUtils.isNotEmpty(searchItem.getPrevItemList())) {
                    for (SearchItem prev : searchItem.getPrevItemList()) {
    ...
    private Expression buildWhereExpression(SearchExpression searchExpression) {
        if (searchExpression.getType() == SearchExpression.Type.EXPRESSION) {
            ComparisonOperator expression = null;
            SearchItem searchItem = searchItemMap.get(searchExpression.getAttr());
            if (searchItem != null) {
                expression = searchExpression.getComparisonExpression();
                if (expression != null) {
                    expression.withLeftExpression(new Column().withColumnName(searchItem.getAlias()))
                            .withRightExpression(searchExpression.getExpressionValue());
                    return expression;
                }
            }
