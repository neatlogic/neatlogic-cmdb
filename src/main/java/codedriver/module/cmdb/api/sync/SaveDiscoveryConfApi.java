...
public class SaveDiscoveryConfApi extends PrivateApiComponentBase {
    ...
    public String getToken() {
        return "/cmdb/sync/discovery/conf/save";
    }
    ...
    @Input({@Param(name = "_id", type = ApiParamType.STRING, desc = "id，存在代表修改，不存在代表新增"),
            ...
            @Param(name = "timingTmpl", type = ApiParamType.STRING, isRequired = true, desc = "??"),
            @Param(name = "workerCount", type = ApiParamType.INTEGER, isRequired = true, desc = "??")
    })
    @Description(desc = "保存自动发现设置接口")
    @Override
    public Object myDoService(JSONObject jsonObj) throws Exception {
        String[] fieldList = new String[]{"_id", "nets", "ports", "snmpPort", "timingTmpl", "workerCount"};
