...
public class SearchCiAttrRelListApi extends PrivateApiComponentBase {
    ...
    public Object myDoService(JSONObject jsonObj) throws Exception {
        ...
        if (StringUtils.isNotBlank(keyword)) {
            ...
            attrList = attrList.stream().filter(d -> d.getName().contains(keyword)).collect(Collectors.toList());
            relList = relList.stream().filter(d -> (d.getDirection().equals(RelDirectionType.FROM.getValue()) && d.getToName().contains(keyword))
                    || (d.getDirection().equals(RelDirectionType.TO.getValue()) && d.getFromName().contains(keyword))).collect(Collectors.toList());
        }
